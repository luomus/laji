...
import { getBoxLabel, getDefaultSampleRate } from '../../../../bsg-shared/service/bsg-utils';
  ...
  private updateSelectedSpeciesAndSpectrogramRectangles() {
    ...
    if (speciesAnnotations && speciesAnnotations.length > 0) {
      const observables: Observable<SpeciesWithAnnotation>[] = speciesAnnotations!.map(
        annotation => this.bsgApi.getSpecies(this.translate.getCurrentLang(), annotation.speciesId, true).pipe(

...
import { RecordingAnnotation, RecordingWithAnnotation } from '../../bsg-shared/models';
  ...
  getCurrentRecording(): Observable<RecordingWithAnnotation> {
    if (this.current !== null) {
      ...
      if (this.dataByRecordingId[this.current]) {
        ...
        data$ = this.bsgApi.getIdentificationRecording(this.userService.getToken(), this.translate.getCurrentLang(), this.current).pipe(
          tap(result => {
  ...
  private initPreloadNextRecordings() {
    this.preloadNextRecording$ = this.preloadNextRecordingSubject.pipe(
      switchMap(previousId => {
        ...
        return this.bsgApi.getNewIdentificationRecording(

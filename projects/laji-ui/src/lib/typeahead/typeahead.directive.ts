...
import { TypeaheadOptionItemContext, TypeaheadOptionListContext, TypeaheadOrder } from './models';
...
/**
 * The luTypeahead directive is copied from ngx-boostrap@cff1e918,
...
export class TypeaheadDirective implements OnInit, OnDestroy {
   ...
   */
  @Input('luTypeahead') typeahead?: TypeaheadOptionArr;
  /** minimal no of characters that needs to be entered before
   * typeahead kicks-in. When set to 0, typeahead shows on focus with full
   * list of options (limited as normal by typeaheadOptionsLimit)
   */
  @Input('luTypeaheadMinLength') typeaheadMinLength = 1;
  /** minimal wait time after last character typed before typeahead kicks-in */
  @Input('luTypeaheadWaitMs') typeaheadWaitMs = 0;
  /** maximum length of options items list. The default value is 20 */
  @Input('luTypeaheadOptionsLimit') typeaheadOptionsLimit?: number;
  /** when options source is an array of objects, the name of field
   * that contains the options value, we use array item as option in case
   * of this field is missing. Supports nested properties and methods.
   */
  @Input('luTypeaheadOptionField') typeaheadOptionField?: string;
  /** when options source is an array of objects, the name of field that
   * contains the group value, matches are grouped by this field when set.
   */
  @Input('luTypeaheadGroupField') typeaheadGroupField?: string;
   ...
   */
  @Input('luTypeaheadOrderBy') typeaheadOrderBy?: TypeaheadOrder;
  /** should be used only in case of typeahead attribute is Observable of array.
   * If true - loading of options will be async, otherwise - sync.
   * true make sense if options array is large.
   */
  @Input('luTypeaheadAsync') typeaheadAsync?: boolean;
  /** match latin symbols.
   * If true the word súper would match super and vice versa.
   */
  @Input('luTypeaheadLatinize') typeaheadLatinize = true;
  /** Can be use to search words by inserting a single white space between each characters
   *  for example 'C a l i f o r n i a' will match 'California'.
   */
  @Input('luTypeaheadSingleWords') typeaheadSingleWords = true;
  /** should be used only in case typeaheadSingleWords attribute is true.
   * Sets the word delimiter to break words. Defaults to space.
   */
  @Input('luTypeaheadWordDelimiters') typeaheadWordDelimiters = ' ';
   ...
   */
  @Input('luTypeaheadMultipleSearch') typeaheadMultipleSearch?: boolean;
   ...
   */
  @Input('luTypeaheadMultipleSearchDelimiters') typeaheadMultipleSearchDelimiters = ',';
  /** should be used only in case typeaheadSingleWords attribute is true.
   * Sets the word delimiter to match exact phrase.
   * Defaults to simple and double quotes.
   */
  @Input('luTypeaheadPhraseDelimiters') typeaheadPhraseDelimiters = '\'"';
  /** used to specify a custom item template.
   * Template variables exposed are called item and index;
   */
  @Input('luTypeaheadItemTemplate') typeaheadItemTemplate?: TemplateRef<TypeaheadOptionItemContext>;
  /** used to specify a custom options list template.
   * Template variables: matches, itemTemplate, query
   */
  @Input('luTypeaheadOptionsListTemplate') optionsListTemplate?: TemplateRef<TypeaheadOptionListContext>;
  /** specifies if typeahead is scrollable  */
  @Input('luTypeaheadScrollable') typeaheadScrollable = false;
  /** specifies number of options to show in scroll view  */
  @Input('luTypeaheadOptionsInScrollableView') typeaheadOptionsInScrollableView = 5;
  /** used to hide result on blur */
  @Input('luTypeaheadHideResultsOnBlur') typeaheadHideResultsOnBlur?: boolean;
  /** fired when an options list was opened and the user clicked Tab
   * If a value equal true, it will be chosen first or active item in the list
   * If value equal false, it will be chosen an active item in the list or nothing
   */
  @Input('luTypeaheadSelectFirstItem') typeaheadSelectFirstItem = true;
  /** makes active first item in a list */
  @Input('luTypeaheadIsFirstItemActive') typeaheadIsFirstItemActive = true;
  /** fired when 'busy' state of this component was changed,
   * fired on async mode only, returns boolean
   */
  @Output('luTypeaheadLoading') typeaheadLoading = new EventEmitter<boolean>();
  /** fired on every key event and returns true
   * in case of matches are not detected
   */
  @Output('luTypeaheadNoResults') typeaheadNoResults = new EventEmitter<boolean>();
  /** fired when option was selected, return object with data of this option. */
  @Output('luTypeaheadOnSelect') typeaheadOnSelect = new EventEmitter<TypeaheadMatch>();
  /** fired when option was previewed, return object with data of this option. */
  @Output('luTypeaheadOnPreview') typeaheadOnPreview = new EventEmitter<TypeaheadMatch>();
  /** fired when blur event occurs. returns the active item */
  @Output('luTypeaheadOnBlur') typeaheadOnBlur = new EventEmitter<TypeaheadMatch>();

  /** Placement of the dropdown relative to the input. */
  @Input('luTypeaheadPlacement') placement: Placement = 'bottom';
  ...
  private _containerRef?: ComponentRef<TypeaheadContainerComponent>;
  ...
  show(): void {
    ...
    if (this._containerRef || !this.ngControl.control || !window) {
      return;
    }

    this._containerRef = this.viewContainerRef.createComponent(TypeaheadContainerComponent);
    this._container = this._containerRef.instance;
    this._container.parent = this;
    this._container.placement = this.placement;
    ...
    this.placementService.attach(
      this._containerRef.location.nativeElement,
      this.element.nativeElement,
      this.placement,
      { renderer: this.renderer, window, document: this.document }
    );
    this._containerRef.changeDetectorRef.detectChanges();
    this.placementService.update(this._containerRef.location.nativeElement);
    this.isOpen = true;
    this.changeDetection.markForCheck();
  }
  ...
  hide(): void {
    if (this._containerRef) {
      this.placementService.detach(this._containerRef.location.nativeElement);
      this.renderer.removeChild(this.document.body, this._containerRef.location.nativeElement, true);

...
@Component({
  selector: 'laji-municipality-monitoring-form',
  templateUrl: './../../common/theme-form.component.html'
})
export class MunicipalityMonitoringFormComponent
  ...
  ngOnInit() {
    this.formId = environment.municipalityMonitoringForm;
  ...
  onAccessDenied() {
    this.translateService.get('form.permission.no-access')
      .subscribe(msg => {
        ...
        this.router.navigate(
          this.localizeRouterService.translateRoute(['/theme/kunnat']),
          { replaceUrl: true }
        );
      });
  }

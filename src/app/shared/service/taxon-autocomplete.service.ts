...
export class TaxonAutocompleteService {
  ...
  getAutocompleteDisplayName(payload: any, text: string) {
    ...
    console.log(this.vernacularName)
    console.log(this.matchingName)

    switch (payload['nameType']) {
      case 'MX.scientificName':
        return (payload['cursiveName'] ? '<i>' + this.scientificName + '</i>' : this.scientificName)
        +' <div> (' + this.labelPipe.transform(payload['taxonRankId']) + ') <div> <div class="' + (payload['informalTaxonGroups'].lenght > 0 ? (payload['informalTaxonGroups'].lenght > 0 ? payload['informalTaxonGroups'][0].id : '') : '') +'"></div>'+ (payload['finnish'] ? '<div></div>' : '<span></span>') + '</div></div>';
      case 'MX.hasSynonym':
        return (payload['cursiveName'] ? '<i>' + this.scientificName + '</i>' : this.scientificName)
         +' (' + (this.matchingName + ') <div>' 
         + (payload['taxonRankId'] ? ' (' + this.labelPipe.transform(payload['taxonRankId']) + ') ' : '') + '<div><div class="' + (payload['informalTaxonGroups'].lenght > 0 ? payload['informalTaxonGroups'][0].id : '') +'"></div>') + (payload['finnish'] ? '<div></div>' : '<span></span>' ) + '</div></div>'
      case 'MX.birdlifeCode':
        return this.matchingName + ' - ' + (payload['cursiveName'] ? '<i>' + this.scientificName + '</i>' : this.scientificName) + '<div class="responsive_container">' + (payload['taxonRankId'] ? ' (' + this.labelPipe.transform(payload['taxonRankId']) + ') ' : '' ) + '<div class="flag_icon"><div class="' + (payload['informalTaxonGroups'].lenght > 0 ? (payload['informalTaxonGroups'].lenght > 0 ? payload['informalTaxonGroups'][0].id : '') : '') +'"></div>' + (payload['finnish'] ? '<div></div>' : '<span></span>') + '</div>';
      case 'MX.euringCode':
        return this.matchingName + ' - ' + (payload['cursiveName'] ? '<i>' + this.scientificName + '</i>' : this.scientificName) + '<div class="responsive_container">' + (payload['taxonRankId'] ? ' (' + this.labelPipe.transform(payload['taxonRankId']) + ') ' : '' ) + '<div class="flag_icon"><div class="' + (payload['informalTaxonGroups'].lenght > 0 ? payload['informalTaxonGroups'][0].id : '') +'"></div>' + (payload['finnish'] ? '<div></div>' : '<span></span>') + '</div>';
  ...
  addBold(original: string, substring: string) {
    substring = original.includes(substring.toLowerCase()) ? substring.toLowerCase() : this.capitalize(substring);

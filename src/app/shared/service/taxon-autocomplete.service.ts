  ...
  getAutocompleteDisplayName(payload: any, text: string) {
    ...
    switch (payload['nameType']) {
      case 'MX.scientificName':
        ...
        +' <div> (' + this.labelPipe.transform(payload['taxonRankId']) + ') <div> <div class="' + payload['informalTaxonGroups'][0].id +'"></div>'+ (payload['finnish'] ? '<div></div>' : '<span></span>') + '</div></div>';
      case 'MX.hasSynonym':
        return (payload['cursiveName'] ? '<i>' + this.scientificName + '</i>' : this.scientificName)
         +' (' + (this.matchingName + ') <div>' 
         + (payload['taxonRankId'] ? ' (' + this.labelPipe.transform(payload['taxonRankId']) + ') ' : '') + '<div><div class="' + payload['informalTaxonGroups'][0].id +'"></div>') + (payload['finnish'] ? '<div></div>' : '<span></span>' ) + '</div></div>'
